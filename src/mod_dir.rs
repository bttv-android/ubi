...
pub fn handle_mod_dir(dx_path: String, baksmali_path: String, dir_path: &String) -> &'static str {
    ...
    let f = match fs::File::open(aar_path) {
        Ok(f) => f,
        Err(e) => panic!("could not open twitch-debug.aar: {}, please run ./buildsource first", e),
    };
